...
const appointmentController = {
  ...
  createAppointment: async (request, response) => {
    ...
    try {
      ...
      const weekDayValue = new Date(appointment.appointmentDate.year, appointment.appointmentDate.month -1, appointment.appointmentDate.day).getDay();
      ...
      if (availabilityQuery.empty){
        return response.status(400).json({ status: "Erro", message: "Não há disponibilidade para o profissional neste dia." }); //seguir esse padrão de responses  
      }
      ...
      if (appointmentDateTime < availabilityStartTime || appointmentDateTime > availabilityEndTime) {
        return response.status(400).json({ status: "Erro", message: "O horário da consulta está fora da disponibilidade do profissional." });
      }

      //verifica outras consultas marcadas
      const appointmentConflictQuery = await admin.firestore().collection("Appointments")
      .where("professionalId", "==", appointment.professionalId)
